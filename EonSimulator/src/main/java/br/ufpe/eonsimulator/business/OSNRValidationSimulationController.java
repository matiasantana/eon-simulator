...
public class OSNRValidationSimulationController extends
	...
	public void run(Simulation simulation) {
			...
			for (IsPhysicalElement isPhysicalElement : physicalElements) {
				for (IsPhysicalElement isPhysicalElement2 : physicalElements) {
					if (isPhysicalElement.isTopologyNode()
						...
						// time of the
						// connection
						connection = simulation.getTrafficGenerator()
								.createConnection(simulation, elementPair);
						...
						if (routes != null && !routes.isEmpty()) {
							...
							double routeDistance = simulationRouteWrapper
		...
		System.out.println("Physical blocking percent: "
				+ ConvertUtils.convertToLocaleString(simulation
						.getSimulationResults()
						.getPhysicalBlockingProbability() * 100));
		System.out.println("Number of Required Slots: "
				+ connection.getNumberSlotRequired());
		if (maxRouteOrderingWrapper != null) {
			System.out.println("Max Route Distance: " + maxDistance);
			System.out.println("Max Route OSNR: "
					+ MathUtils.convertLinearTodB(maxRouteOrderingWrapper
							.getRoute().getOSNR()));
			System.out.println("OSNRreq: "
					+ MathUtils.convertLinearTodB(maxRouteOrderingWrapper
							.getRequiredOSNR()));
			System.out.println("Max Route: "
					+ maxRouteOrderingWrapper.getRoute()
